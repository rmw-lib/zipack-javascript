...
function vlqs2string(zipack, length) {
  const begin = zipack.index
  zipack.index+=length;
  return scsu.decode(zipack.slice(begin, zipack.index))
...
function encodeString(string) {
  // 小字符串还是大字符串
  string = scsu.encode(string)
  var tobeUint8Array =
    string.length < 32
    ? [_string_0 | string.length]
    : [_string, ...nature2vlq(string.length - 32)];

  return Uint8Array.from(tobeUint8Array.concat(string));
}
